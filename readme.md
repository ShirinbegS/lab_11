...
## Генератор мемов
1. Реализовал приложение для генерации мемов и вывода через tkinter
## Код
```
...
def create_mem(bg, mem, mem_txt):
    ...
    combo_image = bg_image.copy()

    x = (bg_image.width - meme_width)//2
    y = (bg_image.height - meme_height)//2

    combo_image.paste(mem_image, (x,y), mask=mem_image.convert("RGBA"))

    draw = ImageDraw.Draw(combo_image)
